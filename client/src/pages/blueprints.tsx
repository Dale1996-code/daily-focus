...
import { Copy, Plus, Check, ChevronRight, Briefcase, Users, MessageSquare } from "lucide-react";
...
export default function Blueprints() {
  ...
  const handleUseBlueprint = (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
    setCopiedId(id);
    // In a real app, this would add these tasks to the day's active tasks list
  ...
  return (
    <div className="p-4 md:p-8 pt-6 pb-24 md:pb-8 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-4xl mx-auto">
          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <button
                  onClick={(e) => handleUseBlueprint(e, bp.id)}
                  className={cn(
                    "hidden sm:flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-300",
                    copiedId === bp.id 
                      ? "bg-green-500/10 text-green-600" 
                      : "bg-primary text-primary-foreground hover:bg-primary/90 hover:scale-95 active:scale-90"
                  )}
                >
                  {copiedId === bp.id ? (
                ...
                >
                  <div className="p-5 md:p-6 space-y-3">
                    <div className="flex items-center justify-between mb-4 sm:hidden">
                      ...
                      >
                        {copiedId === bp.id ? <Check className="w-3 h-3" /> : <Plus className="w-3 h-3" />}
                        {copiedId === bp.id ? "Added" : "Use All"}
                      </button>
                    </div>
                    {bp.tasks.map((task) => (
      ...
      <button className="mt-8 w-full border-2 border-dashed border-border text-muted-foreground font-semibold rounded-3xl p-6 flex flex-col items-center justify-center gap-2 hover:border-primary/50 hover:text-primary hover:bg-primary/5 transition-all group">
